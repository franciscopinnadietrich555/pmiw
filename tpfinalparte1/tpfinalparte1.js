...
//Comision 3
//Link al video: https://youtu.be/bnNS3zs_a18
let impa = [];
let txpa = [];
let estado = "ini";
...
function preload() {
  ...
  for (let i = 1; i < 15; i++) {
    impa[i] = loadImage("data/img" + i + ".jpg", img => {
      console.log("Imagen cargada: data/img" + i + ".jpg");
    }, err => {
      console.error("Error cargando imagen: data/img" + i + ".jpg", err);
    });
  }
